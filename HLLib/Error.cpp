...
const hlChar *CError::GetShortFormattedErrorMessage()
{
	if(this->uiSystemError == 0)
	{
		if(*this->lpError)
		{
			sprintf_s(this->lpShortFormattedError, "Error: %s", this->lpError);
		}
		else
		{
			strcpy_s(this->lpShortFormattedError, "<No error reported.>");
		}
	}
	else
	{
		sprintf_s(this->lpShortFormattedError, "Error (0x%.8x): %s %s", this->uiSystemError, this->lpError, this->lpSystemError);
	}
...
const hlChar *CError::GetLongFormattedErrorMessage()
{
	if(this->uiSystemError == 0)
	{
		if(*this->lpError)
		{
			sprintf_s(this->lpLongFormattedError, "Error:\n%s", this->lpError);
		}
		else
		{
			strcpy_s(this->lpLongFormattedError, "<No error reported.>");
		}
	}
	else
	{
...
hlVoid CError::SetErrorMessageFormated(const hlChar *lpFormat, ...)
{
	...
	vsprintf_s(this->lpError, lpFormat, ArgumentList);
	va_end(ArgumentList);
...
hlVoid CError::SetSystemErrorMessageFormated(const hlChar *lpFormat, ...)
{
	...
	{
		strcpy_s(this->lpSystemError, (hlChar *)lpMessage);
	...
	{
		strcpy_s(this->lpSystemError, "<Unable to retrieve system error message string.>");
	}
}
