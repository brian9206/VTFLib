...
namespace HLLib
{
	...
	private:
		...
		hlChar lpSystemError[4096];

		hlChar lpShortFormattedError[4096];
		hlChar lpLongFormattedError[4096];
...
namespace HLLib
{
	...
	public:
		...
		CDirectoryFolder *AddFolder(const hlChar *lpName, hlUInt uiID = 0xffffffff, hlVoid *lpData = 0);
		CDirectoryFile *AddFile(const hlChar *lpName, hlUInt uiID = 0xffffffff, hlVoid *lpData = 0);
		...
		hlVoid Sort(HLSortField eField = HL_FIELD_NAME, HLSortOrder eOrder = HL_ORDER_ASCENDING, hlBool bRecurse = 1);
		...
		hlUInt GetSize(hlBool bRecurse = 1) const;
		hlULongLong GetSizeEx(hlBool bRecurse = 1) const;
		hlUInt GetSizeOnDisk(hlBool bRecurse = 1) const;
		hlULongLong GetSizeOnDiskEx(hlBool bRecurse = 1) const;
		hlUInt GetFolderCount(hlBool bRecurse = 1) const;
		hlUInt GetFileCount(hlBool bRecurse = 1) const;
